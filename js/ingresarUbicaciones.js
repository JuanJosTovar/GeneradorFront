  ...
  document.getElementById('excelForm').addEventListener('submit', async function(event) {
    ...
    try {
      ...
      for (const [index, row] of jsonData.entries()) {
        ...
        try {
          const res = await fetch('http://localhost:10101/registrarUbicacion', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
          });
          const data = await res.json();
          responses.push(`Fila ${index + 2} procesada (ubicacion: ${ubicacion}): ${JSON.stringify(data)}`);
        } catch (error) {
          responses.push(`Error en fila ${index + 2} (ubicacion: ${ubicacion}): ${error}`);
        }
      }
